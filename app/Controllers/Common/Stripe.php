	...
	public function action($type, $paymentintentid='', $subscriptionid='', $result='')
	{
	    if($type=='1'){
			...
			if($result!=''){
				...
				if($payment){
					$datetime = $result->next_payment_attempt!=null ? formatdate($result->next_payment_attempt, 2) : 'recurring ended';
					send_emailsms_template('8', ['userid' => $payment['user_id'], 'eventid' => $payment['event_id'], 'stallsname' => $payment['name'], 'datetime' => $datetime]);
					send_emailsms_template('9', ['email' => getAdminMail(), 'userid' => $payment['user_id'], 'eventid' => $payment['event_id'], 'stallsname' => $payment['name'], 'datetime' => $datetime]);
				}
			}
	    }
	}
