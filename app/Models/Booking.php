...
class Booking extends BaseModel
{
    ...
    {  
		...
		if(isset($requestdata['gtenddate'])){
			$query->groupStart();
				$query->groupStart()->where(['b.type' => '1', 'e.type' => '1'])->groupStart()->where(['e.end_date >=' => date('Y-m-d', strtotime($requestdata['gtenddate']))])->orWhere(['b.check_out >=' => date('Y-m-d', strtotime($requestdata['gtenddate']))])->groupEnd()->groupEnd();
				$query->orGroupStart()->where(['b.type' => '2', 'e.type' => '2'])->groupStart()->where(['e.end_date >=' => date('Y-m-d', strtotime($requestdata['gtenddate']))])->orWhere(['b.check_out >=' => date('Y-m-d', strtotime($requestdata['gtenddate']))])->groupEnd()->groupEnd();
			$query->groupEnd();
		}
		...
		if(isset($requestdata['ltenddate'])){
			$query->groupStart();
				...
				$query->orGroupStart()->where(['b.type' => '2', 'e.type' => '2'])->groupStart()->where(['e.end_date <' => date('Y-m-d', strtotime($requestdata['ltenddate']))])->orWhere(['b.check_out <' => date('Y-m-d', strtotime($requestdata['ltenddate']))])->groupEnd()->groupEnd();
			$query->groupEnd();
		}
